...
public interface PostRepository extends R2dbcRepository<Post, Long> {
    ...
    @Query("select posts.*, users.name from posts inner join users on users.uid = posts.uid where posts.post_id = :postId")
    Mono<PostWithName> findByIdWithName(@Param(value = "postId") Long postId);

    @Query("select posts.*, users.name from posts inner join users on users.uid = posts.uid where posts.title like :keyword order by posts.post_id limit :limit offset :offset")
